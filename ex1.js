...
(async function myFunction() {
     try {
        ...
        await driver.get('http://search.danawa.com/dsearch.php?query=' + data.keywords + '&originalQuery=' +data.keywords + '&previousKeyword=' + data.keywords + '&volumeType=vmvs&page=1&limit=30&sort=saveDESC&list=list&boost=true&addDelivery=N&tab=main&tab=main')
        await driver.manage().setTimeouts( { implicit: 10000 } );
        // Waiting 30 seconds for an element to be present on the page, checking
        // for its presence once every 5 seconds.
        let class_lists = ["prod_info","prod_name","prod_intro","prod_spec_set","relation_goods_unit","prod_sub_info","prod_pricelist"]
        ...
        for(e of elements){
            ...
            if ( ! await e.findElement(By.className(class_lists[2])).getText()){
                prod_intro = await e.findElement(By.className(class_lists[2])).getText()
            }else{
                prod_intro = await e.findElement(By.className(class_lists[2])).getText()
            }           
            
            prod_spec_set =  await e.findElement(By.className(class_lists[3])).getText()         
            relation_goods_unit = await e.findElement(By.className(class_lists[4])).getText()
            prod_sub_info = await e.findElement(By.className(class_lists[5])).getText()
            prod_pricelist = await e.findElement(By.className(class_lists[6])).getText()
